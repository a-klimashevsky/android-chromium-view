...
public class ContentShellActivity extends ChromiumActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			if (!AndroidBrowserProcess.init(this,
					AndroidBrowserProcess.MAX_RENDERERS_LIMIT)) {
				String shellUrl = ShellManager.DEFAULT_SHELL_URL;
				if (savedInstanceState != null
						&& savedInstanceState.containsKey(ACTIVE_SHELL_URL_KEY)) {
					shellUrl = savedInstanceState
							.getString(ACTIVE_SHELL_URL_KEY);
				}
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		if (activeShell != null) {
			outState.putString(ACTIVE_SHELL_URL_KEY, activeShell
					.getContentView().getUrl());
		}
	...
	private void waitForDebuggerIfNeeded() {
		if (CommandLine.getInstance().hasSwitch(
				CommandLine.WAIT_FOR_JAVA_DEBUGGER)) {
	...
	protected void onNewIntent(Intent intent) {
		...
		if (ACTION_LOW_MEMORY.equals(intent.getAction())) {
			MemoryPressureListener
					.simulateMemoryPressureSignal(TRIM_MEMORY_COMPLETE);
			return;
		} else if (ACTION_TRIM_MEMORY_MODERATE.equals(intent.getAction())) {
			MemoryPressureListener
					.simulateMemoryPressureSignal(TRIM_MEMORY_MODERATE);
			return;
		}
